...
def main():
 ...
 for i,spec in enumerate(specs):
  ...
  done=sorted(parts.glob('*.csv'));atomic_csv(pd.DataFrame({'spec_id':[p.stem for p in done],'status':'complete'}),out/'attempt_ledger.csv');atomic_json({'completed_specs':len(done),'target_specs':108,'operation_sha256':EXPECTED_OPERATION_SHA,'panel_sha256':sha(PANEL),'event_ledger_sha256':sha(EVENTS),'daily_ledger_files':len(done),'strict_candidates':0},out/'coverage.json')
  if len(done)%6==0:print(f'sse_four_asset_state_drawdown_v1 {len(done)}/108',flush=True)
 frames=[pd.read_csv(p) for p in sorted(parts.glob('*.csv'))];res=pd.concat(frames);atomic_csv(res,out/'results_by_cohort_timing.csv');agg=[]
 ...
 a=pd.DataFrame(agg);a['strict_candidate']=a.passes_targets & (a.nonoverlap_blocks>=6) & a.asset_identity_passed;atomic_csv(a,out/'candidate_registry.csv');atomic_csv(a[~a.strict_candidate],out/'elimination_ledger.csv');atomic_csv(a.sort_values(['worst_w24','worst_w12'],ascending=False).head(20),out/'pareto.csv');atomic_json({'schema_version':1,'family':'sse_four_asset_state_drawdown_v1','operation_sha256':EXPECTED_OPERATION_SHA,'specifications':108,'completed':len(a),'cohorts_per_spec':int(a.cohorts.min()),'six_block_gate':bool((a.nonoverlap_blocks>=6).all()),'base_dual_target_passes':int(a.passes_targets.sum()),'strict_candidates':int(a.strict_candidate.sum()),'evidence_tier':'official_sse_raw_plus_official_sse_event_pdfs_etf_only','official_daily_margin_required':False,'panel_sha256':sha(PANEL),'event_ledger_sha256':sha(EVENTS)},out/'manifest.json')
