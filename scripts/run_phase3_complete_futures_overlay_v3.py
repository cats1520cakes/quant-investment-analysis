...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--product',required=True);a=ap.parse_args();family=f'u3_equal_weight__{a.product}';grid=pd.read_csv(GRID);grid=grid[grid.family.eq(family)];etf=pd.read_parquet(EP);etf=etf[etf.code.astype(str).isin(CODES)].copy();etf['date']=pd.to_datetime(etf.trade_date);f=pd.read_parquet(FP);f=f[f.instrument_type.eq('future')&f['product'].eq(a.product)].copy();f['date']=pd.to_datetime(f.trade_date);m=pd.read_parquet(MP);ev=pd.read_csv(DIV);ev=ev[ev.code.astype(str).isin(CODES)];out=OUT/family;parts=out/'parts';parts.mkdir(parents=True,exist_ok=True)
