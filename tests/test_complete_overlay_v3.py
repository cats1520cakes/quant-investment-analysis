...
def test_v4_daily_ledger_checksum_and_identity_slice(tmp_path=None):
 from pathlib import Path
 p=Path('/tmp/v4_daily_ledger_test.parquet') if tmp_path is None else tmp_path/'ledger.parquet';d=pd.DataFrame([{'cash':10.,'margin':20.,'etf_market_value':30.,'nav':60.,'asset_identity_residual':0.}]);d.to_parquet(p,index=False,compression='zstd');assert hashlib.sha256(p.read_bytes()).hexdigest();x=pd.read_parquet(p);assert len(x)==1 and x.asset_identity_residual.abs().max()==0
